...
use super::{ config::{ BinserveConfig, CONFIG_STATE }, routes::{ Type, ROUTEMAP }, tls };

use crate::cli::{ self, messages::{ push_message, Type as MsgType } };
  ...
  match ROUTEMAP.get(req.path()) {
    Some(handler) => {
      ...
      if handler.r#type == Type::File {
        ...
        return Ok(
          NamedFile::open(&handler.response.path)?
            .prefer_utf8(true)
            .use_etag(true)
      ...
      Ok(
        HttpResponse::Ok()
          ...
          .content_type(mime_type)
          .body(body)
      ) // NOTE: should we `stream` body here, testing didn't show much changes?
    }
    None => {
...
pub async fn run_server(config_state: BinserveConfig) -> std::io::Result<()> {
  let mut http_server = HttpServer::new(move || {
    let mut app_instance = App::new()
      .wrap({
        ...
        if config_state.config.enable_logging {
          env_logger
            ::try_init_from_env(env_logger::Env::new().default_filter_or("info"))
            .unwrap_or_default();
      ...
      .wrap({
        ...
        headers_middleware = headers_middleware.add((
          SERVER,
        ...
        if !config_state.insert_headers.is_empty() {
          for (header, value) in config_state.insert_headers.iter() {
            headers_middleware = headers_middleware.add((header.as_str(), value.as_str()));
          }
        }
      ...
      .wrap({
        ...
        let mut middleware = Condition::new(
    ...
    if !static_served_from.is_empty() && static_directory != &PathBuf::new() {
      app_instance = app_instance.service({
        let mut static_file_service = actix_files::Files
          ::new(static_served_from, static_directory)
  ...
  let config_state = BinserveConfig::read(&cli::interface::Cli::parse().config_path)?;
  ...
  if config_state.server.tls.enable {
    ...
    http_server = http_server.bind_rustls_0_23(tls_host, tls_config)?;
  }
