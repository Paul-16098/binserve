...
pub const DEFAULT_CONFIG_FILE: &str = "binserve.json";
...
impl BinserveConfig {
  ...
  pub fn read(config_path: &Option<String>) -> io::Result<Self> {
    let config_file = File::open(config_path.clone().unwrap_or(DEFAULT_CONFIG_FILE.to_string()))?;
  ...
  pub fn generate_default_config() -> io::Result<()> {
    if !Path::new(DEFAULT_CONFIG_FILE).exists() {
      ...
      let mut file = File::create(DEFAULT_CONFIG_FILE)?;
