...
impl StaticFile {
  ...
  pub fn create(path: &PathBuf, handlebars_handle: &(Handlebars, HbsContext)) -> Result<Self> {
    ...
    let mut mime_type = mime_guess2
      ::from_path(path)
      .first()
      .unwrap_or("application/octet-stream".parse::<mime::Mime>().unwrap());
    ...
    if ext.is_some() {
      ...
      if extension == "hbs" {
        ...
        // render the template
        contents = Bytes::from(
          hbs_reg.render_template_with_context(&String::from_utf8_lossy(&contents[..]), hbs_ctx)?
        );
...
pub fn generate_not_found() -> Result<StaticFile> {
  ...
  if !error_page_map.is_empty() {
    if let Some(template_path) = error_page_map.get(&404) {
      let read_file = fs
        ::read(template_path)
        .with_context(|| { format!("Failed to read file {:?}", template_path.to_string_lossy()) })?;
...
pub fn generate_starter_boilerplate() -> io::Result<()> {
  ...
  if !Path::new(DEFAULT_CONFIG_FILE).exists() && !Path::new("public").exists() {
    ...
    let starter_directory: HashMap<PathBuf, Vec<u8>> = HashMap::from([
      ...
      ("public/index.html".into(), include_bytes!("../starter/index.html").to_vec()),
      ("public/404.html".into(), include_bytes!("../starter/404.html").to_vec()),
      ("public/usage.hbs".into(), include_bytes!("../starter/usage.hbs").to_vec()),
      ("public/header.hbs".into(), include_bytes!("../starter/header.hbs").to_vec()),
      // assets directory
      (
