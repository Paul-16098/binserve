...
use super::config::{ BinserveConfig, DEFAULT_CONFIG_FILE };
...
pub fn hot_reload_files() -> anyhow::Result<()> {
  let config_path = &cli::interface::Cli::parse().config_path;
  let config_state = BinserveConfig::read(config_path)?;
  ...
  debouncer.watcher().watch(Path::new(DEFAULT_CONFIG_FILE), RecursiveMode::Recursive)?;
  ...
  for route in ROUTEMAP.iter() {
    ...
    if handler.r#type == Type::Bytes {
      ...
      debouncer.watcher().watch(file_path, RecursiveMode::Recursive)?;
  ...
  loop {
    match rx.recv() {
      Ok(Ok(events)) => {
        for event in events {
          if event.path == abs_config_path {
            ...
            let config = BinserveConfig::read(config_path)?;
      ...
      Err(e) => {
        println!("[!] filesystem watch channel error (binserve hot reload): {e:?}");
      }
      Ok(Err(e)) => {
