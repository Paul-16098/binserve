...
use crate::cli::interface::{ self, Cli };
...
pub fn init() -> anyhow::Result<()> {
  ...
  let mut config = BinserveConfig::read(&Cli::parse().config_path)?;

  // override with cli configurations if any
  let cli_args = interface::Cli::parse();
  ...
  if end_time.as_millis() == 0 {
    push_message(Type::Info, &format!("Build finished in {} μs ⚡", end_time.as_micros()));
  } else {
    push_message(Type::Info, &format!("Build finished in {} ms ⚡", end_time.as_millis()));
  }
